...
## Recommended models
Preferred:
- `claude-opus-4-7-thinking-xhigh` — сложные схемы, индексы под нагрузку.

Alternatives:
- `claude-4.6-sonnet-medium-thinking` — стандартный ревью миграции и схемы.
- `gpt-5.3-codex-high-fast` — генерация SQL DDL по чёткому контракту.
