...
## Recommended models
Preferred:
- `claude-4.6-sonnet-medium-thinking` — стандартные задачи реализации и рефакторинг.

Alternatives:
- `claude-opus-4-7-thinking-xhigh` — сложные state-management решения, несколько фич.
- `gpt-5.3-codex-high-fast` — автономная генерация большого объёма UI по чёткому контракту.
