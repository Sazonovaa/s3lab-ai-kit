...
## Recommended models
Preferred:
- `gpt-5.5-medium` — основной выбор для короткого уточняющего маршрута.

Alternatives:
- `claude-4.6-sonnet-medium-thinking` — если нужен более осторожный анализ формулировки пользователя.
- `gpt-5.3-codex` — если уточнение является частью более крупной .NET задачи.
