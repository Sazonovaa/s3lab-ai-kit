## Обязательный запуск
База путей: все относительные пути в этом файле разрешай относительно директории, где расположен этот `AGENTS.md`. Если kit используется как submodule, используй `tiss.ai.kit.standart/` как базовую директорию.
