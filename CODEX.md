...
Path base: все относительные пути в этих инструкциях ([`AGENTS.md`](AGENTS.md), [`.ai/router.md`](.ai/router.md), [`.ai/skills/`](.ai/skills/), [`.ai/subagents/`](.ai/subagents/), [`.codex/`](.codex/)) разрешай относительно директории, где лежит этот `CODEX.md`. Если этот файл открыт через submodule, используй `tiss.ai.kit.standart/` как базовую директорию.
