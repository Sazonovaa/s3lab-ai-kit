...
Path base: все относительные пути в этих инструкциях ([`AGENTS.md`](AGENTS.md), [`.ai/router.md`](.ai/router.md), [`.ai/skills/`](.ai/skills/), [`.ai/subagents/`](.ai/subagents/), [`.claude/`](.claude/)) разрешай относительно директории, где лежит этот `CLAUDE.md`. Если этот файл открыт через submodule, используй `tiss.ai.kit.standart/` как базовую директорию.
